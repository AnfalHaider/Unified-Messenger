...
```powershell
# Before every push. 1897 tests, ~25s.
...
|---|---|
| `Oversight` (40) · `Session` (17) · `Contracts` (15) · `Analytics` (9) · `Notifications` (9) · `Distribution` (7) | `Ai` → `.Ai` (10) · `Adapters` → `.Adapters` (9) · `Backfill` → `.Backfill` (8) · `Shell` → `.Shell` (6) |
...
| Deciding whether your agent shell is inside an MSIX container | Write the marker **from your shell** and read it from a `Win32_Process`-created process — **not** the other way round. Container reads fall through to the real path when no local copy exists, so an outside-write/inside-read test passes either way and proves nothing. A session brief asserted "not containerised" on exactly that inverted test. Measured 2026-08-28: Claude Code's shell **is** containerised; the redirect target is `…\Packages\Claude_pzs8sxrjxfjjc\LocalCache\Local\`. **Never write under `%LOCALAPPDATA%\UnifiedMessenger` from an agent shell** — it forks the owner's store invisibly, and both paths then read identical. To deploy a build over the installed app, drive the copy from outside the container. |
| A code-built brush that resolves the wrong theme | `Application.Current.Resources` resolves the **app-default** theme, not the element's — and this app themes the *window root*, not the application, so it reads Light even in dark mode. That covers the Fluent neutral/control brushes **and every `Um*` token**, because `Tokens.xaml` declares them inside `<ResourceDictionary.ThemeDictionaries>`. Always go through `ThemeBrushResolver.Resolve(element, key)`; the no-element overload resolves against the window root for genuinely static builders. Eight files had each rolled their own private lookup, which is why the white-in-dark surfaces kept reappearing after each fix (v4.99.61–63). Also: resolve brushes on `ActualThemeChanged`/`Loaded`, **never once in a constructor** — a control being constructed is not in the visual tree, so its `ActualTheme` is `Default` (that baked light tiles into `KpiStatCard` permanently). |
| A `FontIcon` that draws nothing | Its `Glyph` is probably the empty string. A zero-length glyph renders nothing while the control stays present, laid out, focusable and clickable — invisible to a reader, to the suite, and on screen alike. Eight shipped that way from the initial commit; the per-account details button was a 12-pixel sliver of pure padding, and the L1 drill-down behind it was unreachable for the life of the feature. Write glyphs as `"\uXXXX"`, **never as an inline character** — all eight blanks were inline and all 26 escaped ones were intact. Pinned by `DesignScaleTests.NoFontIconShipsWithAnEmptyGlyph`. |
| Wrapping text that clips instead of wrapping | It is in a horizontal `StackPanel`. That measures children with **infinite** available width, so `TextWrapping` can never take effect — the text overflows and is clipped by the parent. Use a `Grid` with `Auto` + `*` columns. The dashboard's AI shift briefing, the Activity insights and the About page's feature list were all cut mid-word with no ellipsis. The one legitimate use is a container that is `HorizontalAlignment="Left"` and must hug its content (the triage toast) — there, give the `TextBlock` a `MaxWidth` instead. |
| A test that is green today and red tonight | Two in `BackfillDedupeStoreTests` were, in opposite directions. One used `DateTimeOffset.UtcNow` and added two hours, assuming one local day — true only away from local midnight, so it went red at 22:00 and green again at 00:00, on CI as much as here. Its sibling used fixed dates that had aged past `PruneStaleEntries`' 45-day cutoff, so both entries were discarded, both calls trivially returned true, and it **passed without exercising the thing it named**. Anchor such tests to a recent local mid-morning: recent enough to survive the prune, far enough from midnight to survive the arithmetic. |
| The suite fails and the app is open | `SecondInstanceActivatorTests` is the documented one, but **`TriagePersistenceServiceTests` is not isolated either** and failed once during v4.99.63 with the app running. Kill the app before every run, not just when you remember. |
