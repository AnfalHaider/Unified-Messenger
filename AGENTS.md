...
| Two `%LOCALAPPDATA%\UnifiedMessenger` folders appear on a dev machine | Not a bug and not a second data store. Launching the app from a process inside an MSIX container (e.g. a shell hosted by a packaged app) makes Windows redirect its `%LOCALAPPDATA%` writes to `…\Packages\<pkg>\LocalCache\Local\UnifiedMessenger`. The `[Registry] Loaded …` log line names the path that session actually used — read it before concluding anything about which copy is live. |
