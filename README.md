...
**Current release:** v4.93.0 (**The new fast WhatsApp reader now actually works — verified against your live accounts.** v4.88.0 shipped it, but testing it against real logged-in accounts showed it was silently finding the wrong data and falling back to the old reader every time, so none of the promised benefit was reaching you. Fixed: it now reads **all ~850 chats per account** with **phone numbers resolved for essentially 100%** of them and **message previews for 82–88%** — versus roughly 60 chats before. (Previews fill in over the first minute while an account finishes syncing; names and waiting-counts are right immediately.) Older: **Housekeeping: about 850 lines of code that nothing used are gone.** No behaviour changes — this removes an unused duplicate of the chat-navigation logic, a chart helper whose chart no longer exists, and several helpers reachable only from their own tests. Two documentation pages that pointed at deleted code were corrected. Older: **An account that has quietly stopped updating now says so.** Accounts whose session isn't healthy get a small chip next to their name — *Starting*, *Stale*, *Scan QR*, or *Failed* — with a plain-language explanation of what to do. Healthy accounts get no chip at all, so the badge means something when you see one; a branch card shows its worst account, so one signed-out account can't hide behind healthy siblings. Settings → Data now also shows **which WhatsApp reader is live**, since the fast reader is designed to fall back silently and that would otherwise be invisible. Older: **Custom URL tabs are now a real browser: type an address, go anywhere, and save the page you land on as its own account.** The address bar in a Custom URL tab used to be a read-only label. It's now editable — type an address, press Enter, and the tab goes there. Once you're somewhere worth keeping, **Save site** adds it to your sidebar as its own account (its own separate sign-in, named after the site). Real service tabs — WhatsApp, Google Business — deliberately keep their read-only address and stay pinned to their own site. Typing something that isn't a web address is refused with a plain explanation rather than being quietly handed to a search engine, and non-web addresses (`file:`, `javascript:`) are refused outright. Older: **Waiting counts now refresh roughly every 25 seconds instead of every 90, and each account tells you whether its numbers are actually live.** Because the app now reads WhatsApp's in-memory copy (v4.88.0), refreshing is cheap enough to do far more often — so the dashboard keeps up on its own. Accounts still on the older reader stay on the slower cadence rather than being hammered. Each account also gets a **status chip** — Live, Starting, Stale, Scan QR, or Failed — so an account that looks connected but has quietly stopped updating is now visibly **Stale** instead of silently wrong. Older: **Every waiting customer now shows what they actually said — and a reply you send from your phone registers almost immediately.** Until now the app could only read message text for the ~60 chats WhatsApp happened to have drawn on screen, because the copy it stores on disk is encrypted; everyone further down your list showed a blank preview. The app now reads WhatsApp's own live, in-memory copy of your chats instead — the same data the page itself is displaying — so **previews work for every customer**, and a reply you send from your phone stops a chat showing as "waiting" without you having to Re-sync. If a future WhatsApp update ever moves things around, the app silently goes back to the old method rather than losing your numbers. Older: **Clicking a waiting customer now actually opens their chat — the real fix.** For months this was hit-or-miss, and every attempt to fix it tweaked *how* the app searched (by name, by number). That was the wrong thing to chase: a trace added over the last few builds proved the app was finding the right person every single time and then failing to open the chat at all — because WhatsApp's chat list simply ignores the kind of click the app was sending. It now sends a full, real mouse-click sequence, so the chat opens whether the contact is saved or not. Older: **Waiting reviews now show the right details** — v4.82.0 got them wrong: it showed the *branch name* where the reviewer's name should be, dumped the address, rating and menu text into the middle of the review, and drew the stars as empty boxes. Each row now reads properly: **the reviewer's name, their star rating, how long they've been waiting, and the review itself** — with long reviews expanded rather than cut off at "… More". Older: **Clicking a review that needs a reply now takes you straight to it — and you can read the whole review without leaving the dashboard.** Each waiting review now shows the **reviewer, their star rating, how long ago they left it, and the full review text** (hover for all of it); clicking one opens that account and **scrolls right to that review, briefly outlined** so you can't miss it. (Google gives reviews no individual link, so the app finds the review on the page instead — if the page is slow it may take a few seconds, and worst case you land on the reviews list.) Older: **Google reviews: your real star rating, the true review total, and pending counts that actually update.** Each Google Business card now shows its **actual Google rating and lifetime review count** — e.g. **4.6 ★ · 239 reviews** — alongside the reply rate. (Google doesn't put either number on the reviews manager page, so the app reads them from your business's own Search listing in the background, a few times a day.) And **reviews now refresh on their own** every few minutes while the dashboard is open: previously the review data only ever updated during a manual Re-sync, which is why a review you'd **already replied to kept showing as "needs a reply"** and the counts looked wrong. Older: **Saved contacts now show their name (not their number) in needs-reply, and clicking them opens the chat cleanly.** The app now reads the saved contact **name** from WhatsApp's local contact store, so a waiting customer saved as "Muzzamil Naaz" shows as that instead of "+92…" — and focus searches by the name, which WhatsApp matches exactly (no number-search loop). Older: **Clicking a saved contact opens their chat instead of getting stuck** — when a waiting number is saved under a name, WhatsApp's search shows it by that name, so the previous version couldn't confirm the match and looped (search box flickering the number in and out). It now trusts the top result of a precise full-number search and opens the chat. (Showing the saved name in the needs-reply list itself is a follow-up.) Older: **Clicking a waiting customer now opens their chat** — even when WhatsApp only knows them by a privacy id. The needs-reply list shows real phone numbers, but under the hood many are anonymous `@lid` chats; the app now passes the **resolved phone number** to the chat search so it can actually find and open them (before, it had only the un-searchable privacy id and gave up — "loading, then nothing"). Older: **About logo actually renders now** — the logo loads from its file path via a BitmapImage (relative `ms-appx` Image sources don't resolve in this unpackaged app, which is why it was blank), matching how avatars and the tray icon load. Plus the earlier About fixes (removed "Free forever"). Older: **Re-sync is faster, and the activity graph updates on its own.** Re-sync now processes several accounts **at the same time** instead of strictly one after another, so it finishes noticeably quicker for multi-account setups. And the **message-count analytics (activity graph) now refresh in the background about every 8 minutes** while the app is open — so the graph keeps up with new messages across all your chats without a manual Re-sync (needs-reply counts already refreshed every ~90s). Older: **Click-to-chat is more reliable, live counts refresh on their own, Re-sync shows a time estimate, and a redesigned About screen.** Four things: clicking a waiting customer now waits long enough for a just-opened account to finish loading before it jumps to the chat (the old ~2.5s window gave up too early — "loading, then nothing"). The command center now **refreshes awaiting counts on its own about every 90 seconds** while the app is open, so you rarely need a manual Re-sync just to see new activity. Re-sync now shows an **estimated time remaining**. And the **About screen is redesigned** with the app logo, cleaner layout, and a one-tap update button. Older: **Unidentifiable "ghost" contacts no longer clutter Needs-reply.** WhatsApp sometimes represents a chat only by an anonymous privacy id — no name, no phone number — which can't be identified or opened. Those now drop out of the needs-reply list entirely (they were showing as meaningless long "+numbers" you couldn't act on). Real customers on the newer privacy addressing — the ones that still resolve to a phone or a name — stay. Older: **No more "Could not open conversation" pop-up.** When you click a waiting customer whose chat can't be auto-opened — e.g. a WhatsApp privacy-id contact with no saved name or number — the app now just opens that account's inbox quietly instead of interrupting you with an error dialog. Older: **Your own "Message yourself" chat no longer shows as a customer needing a reply.** WhatsApp's self-chat (notes to yourself) is just a normal chat keyed by your own number, so it was slipping into the needs-reply list and the counts. It's now excluded from oversight entirely — you never owe yourself a reply. Older: **Click-to-open a waiting customer is now accurate — and never opens the wrong chat.** The previous version could type a customer's id into WhatsApp search and open whatever matched first, which for privacy-id (`@lid`) contacts with no saved name/number sometimes landed on your own Notes chat. Now it only opens a chat it can positively verify — by matching the visible phone number or a real contact name — and otherwise just opens the account inbox instead of guessing. Contacts WhatsApp only knows by an anonymous privacy id (no name, no number) genuinely can't be pinpointed by search, so those open the inbox. Older: **Deleted messages stop nagging, and clicking a waiting customer opens their chat.** Two fixes. When a customer sends a message and then **deletes it for everyone**, the chat no longer stays stuck as "needs reply" — the scan now reads WhatsApp's deleted-message ("recalled") marker straight from the chat list, so it works even before the account has fully synced. And clicking a customer in the **Needs-reply list now actually opens that chat** — it drives WhatsApp's own search to find them, so it works for people far down the list and for unsaved numbers, not just whoever's already on screen. Older: **"Needs reply" now means you actually replied — plus file downloads and a tidier About.** Four fixes. **The "needs reply" count now tracks whether you replied** — it reads the last message's direction, not WhatsApp's per-device unread badge. So the same accounts opened on two computers now **agree**, and a chat you merely *read* on your phone (without answering) no longer flips to "handled". **WhatsApp's own notices stop nagging** — one-way messages from WhatsApp's official account (which you can't reply to) no longer count as a customer awaiting a reply, and no longer inflate your activity totals. **Received files download like in a browser** — WhatsApp media and documents now save with the normal download experience (progress + Save-As) instead of doing nothing. And **"Check for updates" moved to the About screen**, next to the version, where it belongs — it still offers to install a newer version when one exists. Older: **New logo & icon** — a fresh brand: a two-puzzle-piece chat bubble with a speech-dots mark, applied to the app/taskbar/window/installer icon. The redundant wordmark was removed from the sidebar (the title bar already shows the app name), and the About screen shows the new icon. Older: **Updates no longer wipe your history** — a big one. Every app update was **deleting most of your saved data** because the installer's "keep these files" list was stale — it only preserved 4 of the ~10 data stores. So each update erased **response-times** (which is why **Response time / SLA met never filled in** — they reset before they could accumulate), plus KPI-trend sparklines, new-vs-returning history, mark-handled/snooze overrides, and the last-known dashboard snapshot (why cards flashed "waiting for first sync" after an update). The installer now preserves **all** your data (every `.json` store) while still cleaning out old program files — so First Response Time and SLA % will build up over the coming days and **survive future updates**. Older: **Settings & Add-account polish** — the Settings page had a stray subtitle ("About opens from the navigation list on the left"); it now describes the page ("Notifications, appearance, accounts, AI, and data — pick a section on the left"). And in the **Add account** dialog the Platform and Workspace dropdowns now stretch to full width, so all the fields line up. Older: **Activity chart totals now agree across views** — switching the Activity chart between "Hour of day", "Day of week" and "Month" showed slightly different message totals for the same range (e.g. 18,003 vs 18,013), because hour-of-day summed the day×hour matrix while the other two summed a separate daily-total map that had drifted from it. All three views now count from the one matrix, so the totals always match (and are the audited customer-only figures). Older: **Dashboard tuning** — the **"Activity patterns" heading no longer gets clipped** (its account/date filters moved to their own row so the title always fits), and the account cards no longer show a perpetual **"reply speed: measuring…"** on every row — reply speed now appears only once there's real data, and the "longest wait" nudge is tighter. Older: **Google review counts cover more than the first page** — the reviews scrape reads the currently-loaded reviews page (Google caps it at 10 rows); the app now sets **"Rows per page" to its maximum** automatically before reading, so the "needs a reply" count reflects far more reviews. Older: **Re-sync progress bar — now with personality** — the determinate progress bar + percentage now shows **rotating, light-hearted status lines** ("Rifling through Depilex F-11's chat history (3 of 4)…", "Seeing who they left on read…") and uses a *sync* icon instead of a warning triangle while it runs, so the wait feels less like an error and more like it's working. Older: **Re-sync now shows a real progress bar** — Re-sync reloads and re-reads every account's history, which takes a while; it used to sit on a spinner with no sense of how far along it was. It now shows a **determinate progress bar with a percentage** that names the account being read and advances as each account completes. Under the hood the v4.62 line also consolidates the two IndexedDB parse paths into one shared parser (fixing a latent "needs reply" drift on freshly-backfilled accounts) and reorganises the service layer into module folders. Older: **Google Business opens the reviews page, and deleted messages stop nagging** — two fixes. **Google Business** now opens **`business.google.com/reviews`** (the review-oversight surface) instead of bouncing to a raw Google Search results page for your business — that redirect is Google's own behaviour for single-location profiles, so we point straight at the reviews manager; existing accounts are migrated automatically. And a **"needs reply" accuracy fix**: when a customer sent a message and then **deleted it for everyone**, the chat stayed flagged as awaiting a reply forever — the scan now ignores a deleted (revoked) last message. (One related limit remains by design: a reply you send from your **phone** can take a moment to sync into the app's read-only view, so a just-answered chat may still show as awaiting until the next sync — a manual Re-sync forces it.) Older: **"Check for updates" can now install the update** — previously it only told you a newer version was available (with an OK button) but gave no way to apply it. Now, when an update is found, it offers **Install now** — which downloads the installer, verifies its SHA-256, and restarts into the new version. (Automatic silent updates on launch were already working; this fixes the manual path.) Older: **Custom URL tabs now load reliably** — a Custom-URL tab monitors a website you chose, so it's no longer constrained to a single origin: the tab can follow its own redirects and links anywhere (sign-in providers, CDNs, cross-domain hops), which is what left some custom URLs stuck on a blank page. Non-web schemes (`file:`, `ftp:`) are still blocked, and the known messaging platforms (WhatsApp, Google Business) keep their navigation guard. Blocked navigations are now written to the app log for diagnosis. Older: **Adding accounts — fixes (2)** — **the "Back to dashboard" button no longer overlaps the page** — it's now docked into a slim top bar above every open account instead of floating over the top-left of the page. And **Custom URL tabs now load `http://` sites** — the navigation guard used to accept `https` only, so an `http` custom URL (or a site that first hops through http) got stuck on a blank page. Older: **Adding accounts — fixes + a browser toolbar** — **Google Business (and other embedded pages) now load** — the navigation guard used to cancel Google's cross-domain sign-in/redirect hops, leaving a blank page; it now allows each platform's whole registrable domain. Embedded / Custom URL tabs get a **navigation toolbar** (Back · Forward · Reload · Home + address) — Reload also un-sticks a slow page. The **"Web page" platform is renamed "Custom URL (any website)"** with a clear URL field, the Add dialog is now titled **"Add account"**, and **Telegram / Meta Business Suite / Instagram** are removed from the picker for now. Older: **Business report — pick your period** — the report now has a **date-range selector** (This week / Last 30 days / Last 90 days); every number, insight and the copy adjust to the chosen period, and the report is retitled "Business report". Also fixed another Button-background theme bug (the "Showing: …" scope chip). Older: **Needs-reply + KPI card fixes** — the Needs-reply rows were rendering with a light-grey background (unreadable in dark mode); they now use theme-correct card surfaces and are **grouped by branch/account** (most-backlogged first, oldest-waiting first within each). The KPI tiles now have a **visible outline in light mode** (they previously blended into the page). Older: **Small UI fixes** — the sidebar button is now **"Add account"** (was "Add Instance"); the Weekly report's AI headline now **wraps** instead of running off the edge; and the KPI band's helper text (e.g. "unread cleared, across accounts") wraps instead of being cut off. Older: **Nicer empty states** — the command center's first-run / no-accounts state and the Activity chart's no-data state now show a proper centred icon + heading + hint instead of a bare line. Older: **New vs returning customers** — the Weekly report now shows how many customers who messaged this week were **new** (first-ever contact) vs **returning**, plus the returning-customer rate. It's tracked fully on-device per account (phone-based identity, so a saved and unsaved number count as one person) and only appears once the app has watched for a week, so it's honest rather than "everyone looks new on day one". Older: **Light-mode text readability fix (2)** — the Activity chart legend labels could still stay invisible in light mode: that panel draws its legend only once, before its theme had resolved, and fell back to the OS theme (dark). Neutral text now falls back to the app window's theme rather than the OS, and the dashboard panels redraw when the theme resolves or you toggle it. Older: **Light-mode text readability fix** — neutral text across the dashboard's code-drawn panels (the command-center hero line, account-card detail chips, "caught up"/freshness labels, "—" placeholders, the **Activity chart legend & axis labels**, and **Reviews** sublabels) could render near-white and invisible in light mode. All of it is now built from each panel's actual theme via one shared resolver, so it's correct in both light and dark. Older: **Dashboard redesign (part 2) — bento layout** — on wide screens the **Activity patterns** and **Reviews** panels now sit **side by side** instead of stacking down a long scroll, so you see status, activity and reviews together; they reflow to a single column (animated) on narrower windows. Plus tighter card-corner consistency. Older: **Command-center redesign (part 1)** — the dashboard now opens with a **hero status line** that answers the one question at a glance ("You're all caught up" or "3 customers are waiting", with the oldest wait, the account furthest behind, and a one-click jump to the backlog), sized far larger than the supporting KPI tiles so your eye lands there first. Notices no longer stack four-high: **at most one banner shows** (by priority), with a "+N more" count folded onto it. And a **dark-mode fix** — section headers used a hard-coded near-black colour that vanished on dark; every brand colour is now a theme-aware token. Older: **One-click local backup & restore** (Settings → Data & Privacy): save your settings, accounts, analytics, response-time history and custom account icons to a single `.zip` on this device — nothing is uploaded anywhere — and restore it later or on another machine. Sign-in sessions and multi-GB AI models are intentionally excluded; restore replaces the current data and prompts a restart. Older: **Business-hours-aware response times** — when you set a location's working hours (or turn on Quiet hours), the SLA clock **pauses outside those hours**, so a message that arrives at 11 PM and is answered at 9 AM counts as a fast reply, not a 10-hour one. Plus an **AI-narrated headline** on the Weekly report (one encouraging, specific sentence from your local Ollama model — aggregate facts only, degrades to the deterministic summary), a **Save as image (.png)** button on the report for sharing a snapshot, and a once-a-week, fully-local **weekly-report reminder** banner in the command center (no OS scheduled task — the app is always-on; toggle in Settings → Notifications). Older: **Needs-reply aging bands** (<15m / 15m–1h / 1–4h / >4h) so you can triage the backlog by wait time, and a new **Activity → Heat map** mode showing your busiest weekday×hour windows with a plain-language coverage nudge ("Busiest on Saturday, 6–9 PM — that window carries 22% of the week"). Older: **Per-account drill-down**: a chart-icon on each account card opens an "account details" view — that account's caught-up %, awaiting, reply time, SLA met, answered-today, and the list of waiting customers (each a click-through) — before the raw WhatsApp WebView. And **Quiet hours** (Settings → Notifications): mute awaiting-reply alerts overnight; they resume when quiet hours end. Builds on v4.52.0's **KPI micro-trends**: the Caught-up % and Awaiting tiles now show a tiny daily sparkline of their recent history (persisted per day, builds up over ~2 weeks). And the **Weekly report** now includes a **median-first-reply trend chart** (last 7 days). Builds on v4.51.0, where awaiting became **current-state, not date-windowed** — a customer waiting since yesterday no longer drops out of "Today", so the count reflects your real backlog. A card's **awaiting pill is now clickable** → opens the Needs-reply list scoped to just that account (with a "Showing: X ✕" clear chip). And each awaiting row has an **overflow menu** to **mark it handled** (replied elsewhere — drops off until a newer message arrives) or **snooze** it (1h/4h/until tomorrow); both self-expire so the backlog can't be permanently faked, and both immediately update every awaiting count. Builds on v4.50.1.)  ·  _older:_ (Activity-graph fix: same-platform accounts share one brand colour, which made the stacked All-accounts chart look monochrome — the chart now assigns **distinct palette colours** per account (stable across renders) for both bars and legend. Also: First-Response-Time now excludes **pre-tracking backlog** — chats whose customer message arrived before the app started watching an account can't produce misleading multi-day "response times". Plus v4.50.0's **Weekly report** button: a plain-language business report with **anomaly detection** (replies slower than last week, rising backlog, neglected accounts), **comparative call-outs** (busiest / slowest account), SLA & volume trends, and a per-account table — with one-click **Save report (.md)** or **Export data (.csv)**, all local. Builds on v4.49.0.)  ·  _older:_ (Three usability fixes. **Notifications** now group **by account** (not by platform) with a per-section account avatar + name, and every row shows a color dot + account name — so you can tell which of several WhatsApp accounts each alert is from. **Activity graph** in All-accounts mode now **stacks each account's contribution in its own color** with a legend and per-bar breakdown tooltips, and the header shows the in-range total. **Reviews** now list **which** reviews need a reply (reviewer + snippet, best-effort) and the "N to reply" count is **clickable** — it opens that account's Google reviews page. Builds on v4.48.0 data-accuracy fixes.)  ·  _older:_ (Data-accuracy audit fixes: **Messages/day no longer counts group chats, WhatsApp Status, broadcasts, or channels** — only real customer messages (this is what inflated the count to hundreds/day), and the authoritative message-store recount now **replaces** old inflated figures on Re-sync instead of keeping the higher number. Daily buckets use your **local calendar day** (evening messages no longer roll onto the previous day in UTC+ timezones). The Activity-graph **date-range filter now actually works** — hour-of-day reads a per-day×per-hour matrix scoped to the selected range instead of always showing all-time. And **Response time / SLA met exclude pre-existing backlog** — only messages that arrived while the app was watching that account count, so the first sync no longer reports a misleading multi-day FRT. Requires one Re-sync after updating. Builds on v4.47.0 redesigned account cards)  ·  _older:_ (Redesigned account cards with **live per-account detail**: a per-card freshness stamp ("updated 3m ago"), a detail-chip row — **reply ~Xm** (median first-reply time), **N answered today**, **N past your reply target**, urgent, dropped — plus a "longest wait right now" nudge, tooltips explaining every number in plain language, and a **status legend** under the cards. Fixes the contradictory "45 late on a 100% caught-up account" figure — "past target" is now counted from the same live snapshot as the awaiting pill, so the numbers always agree. Also: **shimmer skeleton cards** while the first scan runs, subtle card entrance/reposition motion, and a labeled 7-day sparkline. Builds on v4.46.0 (First Response Time, SLA met %, responsive clickable KPI band). Plus the redesigned command center + KPI band, Activity patterns graph + week-over-week trend, AI shift briefing, durable oversight snapshot, custom account icons, the Google Business Reviews section, and a top Personal button.)
