...
**Current release:** v4.45.0 (Infrastructure + polish: the oversight/backfill data path is now fully decoupled from WebView2 behind **`IInstanceConnection`** (#26); the Personal-overview **memory** card now reports the **honest total RAM** (app process **plus** all `msedgewebview2` children, with a per-process breakdown) instead of under-counting; a new **Settings → Accounts** section lets you change any account's icon without right-clicking the sidebar; and dead drag-reorder code was removed. Plus stress fixtures locking the WebView2 memory-eviction policy at scale. Builds on v4.44.x: the Activity-patterns graph stays as **bars** with **accurate, updating per-message data**, behind the single dashboard **Re-sync** button. Plus the redesigned command center + KPI band, Activity patterns graph + week-over-week trend, AI shift briefing, durable oversight snapshot, custom account icons, the Google Business Reviews section, and a top Personal button.)
