...
**Current release:** v4.44.4 (Keeps the Activity-patterns graph as **bars** (reverting the v4.44.3 line-chart experiment) while retaining the v4.44.2 fix that shows **accurate, updating per-message data** — the hour-of-day histogram is read straight from the message store on each Re-sync instead of a stuck per-conversation count — and the single dashboard **Re-sync** button that refreshes everything. Plus the redesigned command center + KPI band, Activity patterns graph + week-over-week trend, AI shift briefing, durable oversight snapshot, custom account icons, the Google Business Reviews section, and a top Personal button.)
