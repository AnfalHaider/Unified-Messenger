...
**Current release:** v4.46.0 (Real response-speed metrics: the command center now tracks **First Response Time** — the median time from a customer's message to your first reply, measured live from real WhatsApp timestamps — plus **SLA met %** (replies within your target) and **answered-today**, alongside the existing caught-up %, awaiting, messages/day, and busiest-window tiles. The KPI band is now **responsive** (wraps at any width) and the tiles are **clickable** (Awaiting → Needs-reply list, Busiest/Messages → activity graph), with honest tooltips clarifying that "Caught up" measures unread cleared, not reply speed. Builds on v4.45.0 (IInstanceConnection #26, honest total-RAM reporting, Settings → Accounts change-icon, stress fixtures). Plus the redesigned command center + KPI band, Activity patterns graph + week-over-week trend, AI shift briefing, durable oversight snapshot, custom account icons, the Google Business Reviews section, and a top Personal button.)
