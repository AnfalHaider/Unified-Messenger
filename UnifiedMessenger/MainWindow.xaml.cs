...
public sealed partial class MainWindow : Window, IShellUiHost
{
    ...
    // ── Web navigation toolbar (embedded / custom-URL tabs) ──────────────────────────────────────
    ...
    void IShellUiHost.ShowWebNavBar(MessengerInstance? instance)
    {
        ...
        if (instance is null)
        {
            WebNavBar.Visibility = Visibility.Collapsed; // dashboard / settings
            return;
        }

        // Docked bar for EVERY open account (the "Back to dashboard" affordance, no longer overlaid on the
        // page). The browser controls (back/forward/reload/home/address) show only for embedded / custom-URL
        // tabs — WhatsApp is driven by the scraper.
        WebNavBar.Visibility = Visibility.Visible;
        ...
        {
            core.HistoryChanged += OnNavBarStateChanged;
            core.SourceChanged += OnNavBarSourceChanged;
        }
        else if (_navBarWebView is not null)
        {
            _navBarWebView.CoreWebView2Initialized += OnNavBarCoreInitialized;
        }

        UpdateWebNavBarState();
    }

    private void DetachNavBarWebView()
    {
        if (_navBarWebView is { } w)
        {
            w.CoreWebView2Initialized -= OnNavBarCoreInitialized;
            if (w.CoreWebView2 is { } core)
            {
                core.HistoryChanged -= OnNavBarStateChanged;
                core.SourceChanged -= OnNavBarSourceChanged;
            }
    ...
    private void OnNavBarCoreInitialized(WebView2 sender, CoreWebView2InitializedEventArgs args)
    {
        sender.CoreWebView2Initialized -= OnNavBarCoreInitialized;
        if (sender == _navBarWebView && sender.CoreWebView2 is { } core)
        {
            core.HistoryChanged += OnNavBarStateChanged;
            core.SourceChanged += OnNavBarSourceChanged;
            UpdateWebNavBarState();
    ...
    private void OnNavBarStateChanged(CoreWebView2 sender, object args) => UpdateWebNavBarState();

    private void OnNavBarSourceChanged(CoreWebView2 sender, CoreWebView2SourceChangedEventArgs args) =>
        UpdateWebNavBarState();

    private void UpdateWebNavBarState()
    {
        var core = _navBarWebView?.CoreWebView2;
        NavBackButton.IsEnabled = core?.CanGoBack ?? false;
    ...
    {
        if (_navBarWebView?.CoreWebView2 is { CanGoBack: true } core)
        {
            core.GoBack();
        }
    }
    ...
    {
        if (_navBarWebView?.CoreWebView2 is { CanGoForward: true } core)
        {
            core.GoForward();
        }
    }

    private void NavReloadButton_Click(object sender, RoutedEventArgs e) =>
        _navBarWebView?.CoreWebView2?.Reload();

    private void NavHomeButton_Click(object sender, RoutedEventArgs e)
    {
        if (_navBarWebView?.CoreWebView2 is { } core && !string.IsNullOrWhiteSpace(_navHomeUrl))
        {
            core.Navigate(_navHomeUrl);
        }
    }
