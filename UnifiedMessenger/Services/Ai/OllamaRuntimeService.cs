...
public sealed class OllamaRuntimeService : IDisposable
{
    ...
    public async Task WarmupAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            AppLogger.LogWarning("Ollama.Runtime", $"Ollama warmup failed: {ex.Message}");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            AppLogger.LogWarning("Ollama.Runtime", $"Ollama runtime download failed: {ex.Message}");
            progress?.Report(new OllamaRuntimeDownloadProgress
            {
                Phase = "failed",
                Completed = 0,
    ...
    public async Task<bool> EnsureRunningAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            AppLogger.LogWarning("Ollama.Runtime", $"Ollama runtime ensure failed: {ex.Message}");
            ConnectionState = OllamaConnectionState.Error;
            return false;
        }
        finally
    ...
    private void TryStartEmbeddedServer()
    {
        ...
        {
            AppLogger.LogWarning("Ollama.Runtime", $"Failed to start embedded Ollama: {ex.Message}");
        }
    }
    ...
    public void Shutdown()
    {
        ...
        {
            AppLogger.LogWarning("Ollama.Runtime", $"Failed to stop embedded Ollama: {ex.Message}");
        }
        finally
        {
