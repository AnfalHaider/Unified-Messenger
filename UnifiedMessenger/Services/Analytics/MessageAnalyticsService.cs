...
/// <summary>Weekday × hour "busiest times" heatmap for staffing/coverage planning.</summary>
public sealed class WeekHourHeatmap
{
    public static WeekHourHeatmap Empty { get; } = new()
    {
        Grid = [],
        Total = 0,
        PeakDayIndex = -1,
        ...
        PeakValue = 0,
        BusiestDayName = "—",
        BusiestBlockStartHour = -1,
        BusiestBlockSharePercent = 0,
    };

    /// <summary>[weekday Mon=0..Sun=6][hour 0..23] inbound counts.</summary>
    public required int[][] Grid { get; init; }

    public int Total { get; init; }

    public int PeakDayIndex { get; init; }

    public int PeakHour { get; init; }

    public int PeakValue { get; init; }

    public required string BusiestDayName { get; init; }

    public int BusiestBlockStartHour { get; init; }

    public int BusiestBlockSharePercent { get; init; }
...
public sealed class MessageAnalyticsService : IMessageAnalyticsService
{
    ...
    /// <summary>
    /// A weekday × hour "busiest times" heatmap (Mon..Sun × 0..23) of inbound customer messages over the
    /// window — for staffing/coverage planning. Cells hold message counts; also reports the peak cell and,
    /// as a plain-language nudge, the share of the week's volume carried by the busiest 3-hour block.
    /// </summary>
    public WeekHourHeatmap BuildWeekHourHeatmap(
        IEnumerable<MessengerInstance> instances,
        DateTimeOffset? fromUtc,
        DateTimeOffset? toUtc)
    {
        ...
        foreach (var pair in _stats)
        {
            ...
            foreach (var (dayKey, hours) in pair.Value.HourlyReceivedByDay)
            {
                if (hours is not { Length: 24 } ||
                    !DateTime.TryParseExact(dayKey, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out var date) ||
                    !IsDayInRange(dayKey, fromUtc, toUtc))
                {
                    continue;
                }

                var dow = ((int)date.DayOfWeek + 6) % 7; // Mon=0 .. Sun=6
                for (var h = 0; h < 24; h++)
                {
                    grid[dow][h] += hours[h];
                    total += hours[h];
        ...
        if (total == 0)
        {
            return WeekHourHeatmap.Empty;
        }

        // Peak cell + busiest weekday name.
        var peakDay = 0;
        var peakHour = 0;
        var peakVal = -1;
        for (var d = 0; d < 7; d++)
        ...
        // Busiest contiguous 3-hour block on the peak weekday (a schedulable coverage window).
        var blockStart = 0;
        var blockBest = -1;
        for (var h = 0; h <= 21; h++)
        {
            var sum = grid[peakDay][h] + grid[peakDay][h + 1] + grid[peakDay][h + 2];
            if (sum > blockBest)
            {
                blockBest = sum;
                blockStart = h;
        ...
        var blockSharePercent = (int)Math.Round(blockBest * 100.0 / total);

        return new WeekHourHeatmap
        {
            Grid = grid,
            Total = total,
            ...
            PeakHour = peakHour,
            PeakValue = peakVal,
            BusiestDayName = dayNames[peakDay],
            BusiestBlockStartHour = blockStart,
            BusiestBlockSharePercent = blockSharePercent,
        };
    }
