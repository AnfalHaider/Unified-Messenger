...
public sealed partial class WebViewProfileManager : IWebViewProfileManager
{
    ...
    public Task<WebView2> CreateWebViewAsync(
        ...
        CancellationToken cancellationToken = default,
        bool allowAllNavigation = false) =>
        UiThreadRunner.RunAsync(() => CreateWebViewCoreAsync(profileName, startUrl, cancellationToken, allowAllNavigation));

    private async Task<WebView2> CreateWebViewCoreAsync(
        string profileName,
        string? startUrl,
        CancellationToken cancellationToken,
        bool allowAllNavigation)
    {
        ...
        return await InitializeWebViewControlAsync(environment, profileName, startUrl, cancellationToken, allowAllNavigation)
            .ConfigureAwait(true);
    }
    ...
    private static Task<WebView2> InitializeWebViewControlAsync(
        ...
        UiThreadRunner.RunAsync(async () =>
        {
            ...
            try
            {
                ...
                return FinalizeWebViewControlOnUiThread(webView, profileName, startUrl, allowAllNavigation);
            }
            catch
            {
        ...
        bool allowAllNavigation)
    {
        ...
        {
            if (allowAllNavigation)
            {
                // Custom-URL / generic tab — the user chose an arbitrary site; let it navigate freely so its
                // own redirects/links (OAuth, CDNs, cross-domain hops) aren't cancelled to a blank page.
                WebViewNavigationGuard.AttachAllowingAllHosts(webView.CoreWebView2);
            }
            else
            {
