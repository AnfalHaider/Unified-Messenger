import { band, chip, clock, fig, ic, meter, shell } from './lib.mjs';
...
<div style="display:grid;grid-template-columns:minmax(0,1.35fr) minmax(0,1fr);gap:26px;flex:1;min-height:0">
  <div class="col" style="gap:10px"><div class="row"><h2 class="h2">Messages by hour</h2><span class="sub" style="margin-left:auto;font-size:12px">average day · busiest 12 to 2 pm</span></div>
  <svg viewBox="0 0 700 250" width="100%" height="250">${HOURS.map((v, i) => `<rect x="${36 + i * 47}" y="${220 - v * 5}" width="32" height="${v * 5}" fill="${v >= 33 ? '#2E3191' : '#C9CCEC'}"/><text x="${52 + i * 47}" y="238" text-anchor="middle" font-size="11" fill="#6C7488" font-family="IBM Plex Mono">${9 + i}</text>`).join('')}<line x1="28" y1="220" x2="692" y2="220" stroke="#CCD2E0"/></svg>
  <span class="sub" style="font-size:12px">Two thirds of the week's messages arrive between 11 am and 4 pm.</span></div>
  ...
  ${[['F-11 Markaz', '612', '79%', '14 min', 'late'], ['DHA Phase 2', '438', '88%', '9 min', 'due'], ['Gulberg', '234', '96%', '6 min', 'ok']].map(([l, c, o, m, k]) =>
    `<div class="col" style="gap:7px;padding:13px 14px;border-bottom:1px solid var(--line)">
      ...
      ${meter(parseInt(o, 10), k)}
      <div class="row" style="font-size:12px"><span style="color:${k === 'late' ? 'var(--late)' : k === 'due' ? 'var(--due)' : 'var(--ontime)'};font-weight:600">${o} on time</span><span class="sub" style="margin-left:auto">${m} median first reply</span></div></div>`).join('')}
  ...
  <div class="col" style="gap:18px">
    ...
    <div class="col" style="gap:10px">
      ...
      <div class="col" style="gap:10px;color:var(--ink-2);max-width:44em">
        ...
        <div class="row" style="align-items:flex-start;gap:12px"><span class="mono" style="color:var(--ontime);font-weight:600;font-size:12.5px;padding-top:2px">ALL</span><span>DHA Phase 2 answered every review within a day.</span></div>
...
export const Assistant = () => shell({ tb: { aiOn: true }, active: 'center', content: commandCenterContent({ strip: '' }),
  panel: `<aside class="panel-r"><div class="row" style="padding:13px 16px;border-bottom:1px solid var(--line)">${ic('spark', 17, 'var(--brand)')}<h2 class="h2">Ask</h2>${chip('neu', 'On this PC · gemma3 4B')}<span style="margin-left:auto;color:var(--ink-3)">${ic('x')}</span></div>
  <div class="col" style="flex:1;padding:16px;gap:14px;overflow:hidden">
    <div style="align-self:flex-end;background:var(--brand);color:#fff;border-radius:8px 8px 2px 8px;padding:8px 12px;max-width:85%">Who has been waiting the longest?</div>
  ...
  <div class="col" style="padding:12px 16px;border-top:1px solid var(--line);gap:8px">
    ...
    <div class="input" style="height:38px"><span>Ask about your customers…</span><span style="margin-left:auto;color:var(--brand)">${ic('send', 16)}</span></div>
...
export const Notifications = () => shell({ active: 'center', content: commandCenterContent({ strip: '' }),
  ...
  ${[['clock', 'bad', 'Sara M. passed the reply target', 'WhatsApp · Bookings · F-11 Markaz', '2 min ago', true], ['star', 'bad', 'New 1-star review', 'Google · DHA Phase 2', '2 hours ago', true],
    ...
    <span style="color:${k === 'bad' ? 'var(--late)' : k === 'warn' ? 'var(--due)' : k === 'ok' ? 'var(--ontime)' : k === 'info' ? 'var(--info)' : 'var(--ink-3)'}">${ic(i, 15)}</span>
