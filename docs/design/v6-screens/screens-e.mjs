...
${[['chat', 'WhatsApp', 'Who is waiting, reply times, previews', true], ['chat', 'WhatsApp Business', 'The same, for Business app numbers'], ['insta', 'Instagram', 'Who is waiting in your DMs'], ['google', 'Google reviews', 'Ratings and reviews to answer'],
  ...
  `<div class="row" style="align-items:flex-start;padding:11px;border-radius:5px;border:1px solid ${on ? 'var(--brand)' : 'var(--line)'};${on ? 'background:var(--wash)' : ''}">${ic(i, 17, 'var(--ink-2)')}<div class="col" style="gap:1px"><span style="font-weight:600;color:var(--ink);font-size:12.5px">${t}</span><span style="font-size:12px">${s}</span></div></div>`).join('')}
</div>
...
<div class="col" style="gap:5px"><span class="label">Icon</span><div class="row">${['chat', 'phone', 'building', 'star'].map((i, k) => `<span style="width:36px;height:36px;border-radius:5px;display:grid;place-items:center;border:1px solid ${k === 0 ? 'var(--brand)' : 'var(--line)'};color:var(--ink-2)">${ic(i, 17)}</span>`).join('')}<span class="btn ghost">Use the account's own picture</span></div></div>
...
<div class="col" style="gap:9px;padding:12px;border-radius:5px;background:var(--field)">
 <div class="row" style="align-items:flex-start">${ic('check', 15, 'var(--ontime)')}<span>The next time her PC is online, the app signs her out and wipes every account login on it.</span></div>
