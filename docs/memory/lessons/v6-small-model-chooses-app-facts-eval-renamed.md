---
id: v6-small-model-chooses-app-facts-eval-renamed
date: 2026-09-19
agent: claude
...
files: [v6/core/assistant-summary.ts, v6/scripts/assistant-eval.ts, v6/tests/assistant-set.ts]
cost: Four design rounds against the real model to go from 24 to 30 of 30.
status: live
evidence: npm run assistant:test on 2026-09-19: 30 of 30 with gemma3:4b in 167 s, and 10 of 10 reworded questions outside the set
supersedes: [v6-small-model-chooses-app-facts]
---

Same lesson as `v6-small-model-chooses-app-facts`, restated because the run it names moved: the real-model run is `v6/scripts/assistant-eval.ts` now (lesson `v6-node-test-runs-dash-test-files`), still started with `npm run assistant:test`.
