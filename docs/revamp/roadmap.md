...
location filter, all of Phase 5 (the assistant, 30 of 30 on the real model), and Phase 6 so far: 6.1 Google sign-in, 6.2 the Firestore rules (tested and live), 6.3 workspaces and the shared setup, 6.4 members, 6.5 suspension and the owner console, and 6.6 the public pages (written and tested; publishing waits on one sign-in), each installed on the owner's PC. **Next step: 3.1b** (the Google reviews API reader, built and off until Google approves), in the release plan below, unless an owner decision
