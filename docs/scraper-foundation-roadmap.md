...
|---|---|---|---|
| **A1** | 108 | **Finish Phase 1 on the three live channels.** Google Q&A surface (never inventoried). WhatsApp archived view and the `last-msg-status` ack glyph states — which may make `CanReadAck` cheaper to earn than its comment claims. Messenger Requests / Spam / Archived read from the LS `folderName` column with no navigation. Resolve the `moduleCount: 0` oddity in `__umStoreBridgeProbe`. Docs only. | Done — live sessions exist |
| **A2** | 109 | **Manifest schema + loader.** `Assets/Config/selectors/<platform>.json`: schema version, supported client-version range, ordered fallback candidates per anchor. Loader with a **compiled-in default** so a manifest that fails to load degrades to working code, never to a dead scraper. | A1 |
| **A3** | 110 | **Migrate WhatsApp onto the manifest — the proof.** Every selector out of `whatsapp-adapter.js`, `whatsapp-store-bridge.js` and `adapter-core.js` and into the manifest. If the schema cannot express what the 76 KB adapter already does, the schema is wrong and this is where we find out — before three more channels depend on it. | A2 |
| **A4** | 111 | **Self-diagnosis.** Per-scan report of which anchors matched and which did not, plus the **fallback index that hit** — a rising index is the earliest possible warning of a client redesign. Health surface extending the existing Settings → Data "Store bridge" line, distinguishing *working* / *degraded — running on fallback N* / *broken — anchor X unmatched for Y scans*. A degraded read must never look like a quiet period: honour the existing `IsStale` vs `ReadFailed` split everywhere. Owner's screen and `app.log` only — never off-box. | A3 |
