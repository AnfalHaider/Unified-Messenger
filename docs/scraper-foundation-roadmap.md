...
| ~~**B2a**~~ | ✅ **Answered 2026-09-06, v5.0.1 — `instagram.com/direct/inbox/` does NOT open a conversation.** Shipped as the queue's Instagram click-through and owner-verified on live accounts: the list renders, the search filters, no thread opens. Unlike `messenger.com`, whose root was measured redirecting *into* a conversation — the analogy that made this route look dangerous. **Consequence: preview text and threads beyond the top 15 are both reachable on that route**, so taking them is now a design decision rather than a blocked one. ⚠️ Still unobserved: nobody watched a customer's device, so *"does landing on the inbox mark anything as seen?"* is not answered — only that **no thread was opened**, and Instagram fires its receipt on thread open. | ~10 min | — |
...
| ~~**B6**~~ | ✅ **Done 2026-09-05, build v4.99.98 — all five passes clean.** The owner ran every pass with Narrator and heard nothing wrong: dashboard tab order, the needs-a-reply queue, the read strip, the Reviews queue, and Settings plus the Add-account dialog. First by-ear confirmation this product has ever had, and it also **retracted a stale "known issue"** — Escape *does* close the Add-account dialog, contrary to what the script had asserted for several revisions. Two defects were fixed by scanning in the hour before, so the session did not spend ears on them: review rows announcing only "button" (v4.99.97) and the hub status line overlapping its own header (v4.99.98). Results and the caveat on what "clean" means: [accessibility-listening-session.md](accessibility-listening-session.md). | ~20 min | **Two gaps in one pass.** Everything in the accessibility work is right by construction and by test and nobody has ever *listened* to it — the single largest untested area. It also settles tab order, which cannot be certified from outside: focus rings are one or two pixels and do not survive screenshot scaling, whereas Narrator *announces* each focused control. Independent of B1–B5; can be done first. |
